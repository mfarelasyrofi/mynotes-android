...
public class MainActivity extends AppCompatActivity implements LoadNotesCallback {
    ...
    final ActivityResultLauncher<Intent> resultLauncher = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(),
            result -> {
                if (result.getData() != null) {
                    // Will be called if the request code is ADD
                    if (result.getResultCode() == NoteAddUpdateActivity.RESULT_ADD) {
                        ...
                        showSnackbarMessage("Notes add successfully");
                    }
                    /*
                    will be called if the result code is UPDATE
                    All data is reloaded from the beginning
                    */
                        ...
                        showSnackbarMessage("Notes updated successfully");
                    }
                    /*
                    Will be called if the result code is DELETE
                    Delete will remove data from the list based on the position
                    */
                        ...
                        showSnackbarMessage("Notes deleted successfully");
                    }
                }
            });
    ...
    public void postExecute(ArrayList<Note> notes) {
        ...
        if (notes.size() > 0) {
            ...
            showSnackbarMessage("No current data");
        }
    }
